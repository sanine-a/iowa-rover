...
void setup() {
	Serial.begin(115200);
	int1 = sch.setInterval([]{
		Serial.println("hello from interval 1!");
	}, 1000);
	int2 = sch.setInterval([]{
		Serial.println("this is interval 2!");
	}, 500);
	time1 = sch.setTimeout([]{
		Serial.println("we are now clearing interval 1! >:o");
		sch.clearInterval(int1);
	}, 5000);
	sch.setTimeout([]{
		Serial.println("we will keep interval 1 alive forever!!!!");
		sch.clearTimeout(time1);
	}, 4000);
}
